...
- `workspace-publish`: This script will be called by Travis on tag. You can add `if [ -n "$TRAVIS_TAG" ]; then node node_modules/.bin/workspace-publish; fi` in the `scripts` section of your `.travis.yml` for this. Read on for further details and a working example.
